...
// It seems my previous replace regex failed to match the analyze prompt due to the changing text.
// I will use a more robust replacement strategy.
serverFile = serverFile.replace(/const prompt = `Analyze this resume against the target role of "\$\{targetRole\}".*?Resume:\n\$\{resumeText\}`;/s, 'const prompt = `' + newAnalyzePrompt.replace(/\${/g, '\\${') + '`;');
