...
const startMarker = 'const prompt = `Analyze this resume against the target role';
const endMarker = 'Resume:\n${resumeText}`;';

const startIndex = serverFile.indexOf(startMarker);
const endIndex = serverFile.indexOf(endMarker) + endMarker.length;

if (startIndex !== -1 && endIndex !== -1) {
    const beforePrompt = serverFile.substring(0, startIndex);
    const afterPrompt = serverFile.substring(endIndex);
